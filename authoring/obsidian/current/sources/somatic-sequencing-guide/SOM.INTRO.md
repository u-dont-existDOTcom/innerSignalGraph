---
authoring_contract: inner-signal-authoring-projection-v1
entity_type: source-section
projection_mode: current
generated: true
source_id: SOM.INTRO
guide_id: somatic-sequencing-guide
heading: Introduction
source_role: primary-framework
source_hash: 952e1fd40afa00fd5c762bcc5c8cfbfa8811b66660a90f5495f41f50c69d2428
section_hash: 072ffc6941ab4cab9486cf510d4421d3e4b68470b03505a3ce4822312ea36c5b
locator_kind: text-lines
cited_by_node_ids: []
projection_input_sha256: f92fac6d9a09658ed5bdf982583a7f102b5005e9145db5db141020e10a873b87
---

# Introduction

> [!warning] Generated source-section excerpt — the referenced current-source bytes remain authoritative for the compiled graph.

Authority path: `guides/somatic-sequencing-guide.txt`

Locator: Lines 1–24

```text
Introduction
Trauma healing is rarely just a matter of revisiting painful memories. The body was involved in surviving what happened, and it may still brace, collapse, freeze, disconnect, scan for danger, or flood long after the danger is gone.

For many people, especially people with complex or developmental trauma, it is useful to think in several layers that can support each other: understand body cues and capacity, regulate or discharge activation, explore feelings with compassion, work with
```
