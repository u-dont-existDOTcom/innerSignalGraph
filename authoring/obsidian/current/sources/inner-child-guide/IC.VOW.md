---
authoring_contract: inner-signal-authoring-projection-v1
entity_type: source-section
projection_mode: current
generated: true
source_id: IC.VOW
guide_id: inner-child-guide
heading: Make a Simple Vow
source_role: primary-framework
source_hash: a481cc657ea6e92761a90019a33af9fc6b926037583524f58bbb4dc4953297b3
section_hash: ed810cee5e2a3e7e8e3de7025a0b1abf85d2b17543aedcd65cdb908c3d8e1664
locator_kind: text-lines
cited_by_node_ids:
  - IC.CREDIBILITY_REPAIR
projection_input_sha256: c5c4aed6b8851992ca36c580a60360aeef78931bfaf972429487e3d3143f8084
---

# Make a Simple Vow

> [!warning] Generated source-section excerpt — the referenced current-source bytes remain authoritative for the compiled graph.

Authority path: `guides/inner-child-guide.txt`

Locator: Lines 148–163

```text
Make a Simple Vow
The central connection begins with a quiet promise from the adult to the child, with or without a visualization of the two of you playing on a swing:

“I know you’re in there. I know I’ve ignored you, and I know you don’t trust me. That’s okay. You don’t have to do anything. My job, starting today, is to build a life that is safe for you. I will start listening. I will start protecting us.”

You take that position again whenever the child appears. Trust grows from consistency,
```
