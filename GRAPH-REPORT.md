...
## v0.9.3 runtime planning/realization hardening

The graph source bundle remains r5; v0.9.3 changes the planner/realization contracts around it:
- internal assessments remain attributed to the part/position making them rather than becoming objective facts;
- credibility can be the primary blocking job while regulation remains a supporting job;
- younger-state distrust and resentful-side accusations are treated symmetrically as data;
- selected primary/display-secondary interventions must be materially realized, with one automatic renderer retry on coverage failure;
- the graph-owned substantive question remains canonical.

Development observability is now first-class: CLI development mode stores full local ledgers, the browser exports chat + matching reasoning ledgers as a ZIP without credentials, and lightweight owner feedback is persisted as a self-contained development case. A `Needs work` rating automatically queues a read-only Codex diagnosis in the background.
