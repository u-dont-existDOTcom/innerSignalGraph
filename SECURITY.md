...
Inner Signal is a private, critical-risk application that processes local therapy and hypnosis interactions. Security reports must preserve both credential and user-data boundaries.

## Private reporting

Authorized collaborators should create a draft security advisory in this repository’s **Security → Advisories** area or contact the repository owner through the already established private collaboration channel. If neither route is available, open only a metadata-only private-repository issue asking for a private contact path; do not include exploit details or sensitive material.
