...
test('entire three-condition smoke and replay execute through fake transport only',async t=>{
 ...
 const fetchImpl=async(url,init)=>{
  ...
  if(system.startsWith('You independently review')){
   ...
   text=JSON.stringify({user_goal:'Fixture planning',current_issue:'transport test',turn_task:null,path_update:null,relational_readiness:null,direct_observations:[],variables:{...blankCaseVariables(),present_safety:'safe',orientation:'oriented',ability_to_stop:'yes',ability_to_return:'yes',suicidal_state:'absent',altered_state:'sober',dissociation:'none',activation:'low',actionable_problem:'present',current_intent:'conversation'},hypotheses:[],unknowns:[]});
  }else if(system.includes('adversarial case-formulation auditor')){
   text=JSON.stringify({corrected_turn_task:null,invalidate_turn_task:false,corrected_relational_readiness:null,invalidate_relational_readiness:false,remove_observation_ids:[],remove_hypothesis_ids:[],variable_corrections:[],add_unknowns:[],safety_flags:[],verdict:'accept',summary:'Synthetic transport only'});
