...
test("Canvas deterministically mirrors all graph records and separates overlays", async () => {
  ...
  assert.equal(canvas.nodes.filter((node) => node.type === "file").length, 71);
  assert.equal(canvas.nodes.filter((node) => node.type === "text").length, 13);
  assert.equal(canvas.edges.length, 95);
