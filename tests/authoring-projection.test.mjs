...
test("current projection has the exact canonical inventories and resolved links", () => {
  ...
  assert.equal(keys.filter((item) => item.includes("/nodes/")).length, 73);
  assert.equal(keys.filter((item) => item.includes("/edges/")).length, 99);
  assert.equal(keys.filter((item) => item.includes("/sources/")).length, 116);
  assert.equal(keys.filter((item) => item.includes("/regressions/")).length, 30);
  assert.equal(keys.filter((item) => item.includes("/governance/amendments/")).length, 38);
