...
- A second independent review identified that public hosted-control drift was only warned, so scheduled policy CI could stay green after an applicable control became disabled. The causal mutation test failed 1/1 with all thirteen then-recorded control keys undetected. The repaired profile now records Dependabot security updates explicitly and the audit requires the exact fourteen-key public/completed map, with only installed-App permissions unverified; focused control/evidence tests pass 2/2 while private/pre-public states retain warning semantics.
