...
The live bakeoff passed exact-model probes and completed two full A001 A/C/D replicates plus A replicate 3. Control replicate 2 exercised its existing mandatory-node retry; no advisory retry occurred. A third C extraction then received two identical Claude API 429 envelopes. This is a provider usage limit, not auth or entitlement; the envelope reported a 2026-08-24 20:40 local reset. All completed calls remain in the exact-prompt private provider cache.

The five subsequent release-blocking review findings are repaired in the candidate implementation and focused causal tests pass. Corrected D runs raw semantic formulation first while retaining the established deep Opus/Codex reasoning path and forensic candidate/critique/adjudication council. Both direct safety variables and real graph `blockedNodes` now force the unchanged current hard-authority renderer, preventing unrestricted prose integration across a blocked intervention. Selection and reporting preserve observed-owner, counterfactual-owner, synthetic-engineering, and routing-regression evidence boundaries. The global production subprocess change was reverted exactly to `origin/main`; benchmark-only provider supervision now owns process-tree cleanup. CLI and general environment examples both retain `THERAPY_SCAFFOLD_MODE=current`.

The refreshed harness uses a new implementation fingerprint, resumes all producer and judge calls only by exact prompt/model fingerprints, invalidates malformed structured results so failed stages rerun, preserves raw versus hard-failure-adjusted winners plus judge/order disagreement, scans partial private-text windows across tracked and untracked candidate surfaces, and records a sanitized `BLOCKED` checkpoint if a provider usage limit recurs. No post-repair live provider call has been made yet.

### Current task resume action

Commit the reviewed-finding repairs, then run `npm run experiment:therapy-scaffold -- --benchmark-only --concurrency=2`. The refreshed run identity will reconstruct response artifacts without reissuing exact cached provider calls, while rerunning stages whose prompt/model fingerprint changed or whose prior result failed. Select C, D, or no change only from complete blinded evidence; then freeze the supported candidate, run independent exact-diff reviews and every final gate, and open a draft PR without merging it.
