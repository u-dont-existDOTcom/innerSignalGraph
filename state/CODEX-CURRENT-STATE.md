...
## Maximum-cycle private v3 awaits a fresh independent audit (draft PR #49)
...
Candidate v3 `candidate:repair:abdd6d89-68fa-4e4e-bb89-96b7d206327b` is immutable version 3, repair cycle 2, `reconstructed_pending_audit`, parented to exact v2 and rooted at v1. Private preservation assertions prove its only candidate-text delta is the owner-authorized safeguard and that all unaffected case and trajectory surfaces remain unchanged. V3 has zero audits, no approval, and no sent marker.

The authoritative fresh-session bootstrap is `load_handoff({"handoff_id":"handoff:da297f25-1ae4-4494-b8e1-63591e438d88"})`. Handoff schema v2 separately preserves the raw transcript, provenance-bearing amendments, effective transcript, candidate lineage, and delivery gate. It is continuation-safe and returns exact v3 through the deployed read-only service; a newly created ChatGPT conversation received only this ID and confirmed version 3, repair cycle 2, `reconstructed_pending_audit`, zero audits, not-approved/not-sent state, and the fresh-audit gate without quoting private content or performing a mutation. The next action is `FRESH_INDEPENDENT_AUDIT` and delivery is blocked. A fresh authorized ChatGPT session whose context differs from the producer must audit the exact returned bytes and every repair-induced-error check before any approval. A substantive failure at repair cycle 2 routes to `DISCRIMINATE_OR_BLOCK`, never a third repair. Do not audit from public summaries, approve, send, merge PR #49, deploy the public app, install/promote stable, publish private content, or claim clinical validation.
...
New immutable handoffs preserve candidate ID/version, parent/lineage, status, version-bound audit state, earlier findings, separate handoff/candidate/reconstruction fidelity, and the pending-fresh-audit delivery block. The existing real candidate `candidate:pending:50804229-a5b2-4760-b956-4e5926a56051` remains the current unsent original candidate with no parent, status `pending_audit`, no current audit approval, and delivery blocked. This task does not audit, reconstruct, approve, or send its private text. Focused checks are green and the complete Node 24.18.0 package gate passes 1,047/1,047 tests. Closeout additionally requires `workflow-policy`, `deterministic-package`, `codeql-javascript`, and nested `CodeQL` to be green on the exact current PR head. Keep PR #49 draft/open/unmerged and do not deploy the public app, install, promote stable, publish private content, or claim clinical validation.
