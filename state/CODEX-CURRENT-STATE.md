...
The additive task contains one nine-turn fictional case, 15 seeded flawed drafts covering 25 original-plus-longitudinal failure classes, two neutrally named already-good termination controls, runtime-like prompt contracts for A–D plus optional E and no-audit baseline, content-bound B patch-versus-reconstruct pairing, repeat-aware opaque scheduling, schema-validated canonical stage outputs and grading evidence, topology-derived findings/actions/responses, raw evidence-bearing initial/final grades, full-schedule comparison gating, deterministic pooled/macro scoring, privacy/schema/mutant regressions, and deterministic ChatGPT audit/grader packets. Two independent read-only reviews drove information-firewall, regression-attribution, no-op, calibration, topology, pairing, schedule, cost, comparison, and audit-output provenance repairs. Role-separated blinded batch packets now preserve two fresh grader passes while binding unique logical grades to shared physical-submission IDs and raw-output hashes. The combined focused harness suite passes 43/43 and the full repository gate passes 994/994 on pinned Node 24.18.0. Provider/API/OpenRouter calls remain zero and no real-person identifier, transcript excerpt, source conversation ID, exact personal amount/location, or private-derived hash is stored.
