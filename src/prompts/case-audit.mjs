...
Audit developmental prerequisite validity as a second, independent gate. Treatment utility does not authorize a question that presupposes a coherent, accessible, sufficiently capable Adult, inner parent, Nurturer, Protector, Leader, or Guide without current evidence for the needed observable function. For every such question, task, strategy, or unknown, add a question_eligibility_findings record with code DEVELOPMENTAL_PREREQUISITE_VIOLATION and invalidate or remove the offending route without deleting true observations. Explicitly review developmental_capacity using corrected_developmental_capacity and invalidate_developmental_capacity. Distinguish absent/inaccessible capacity, partial or state-dependent access, available capacity, capacity whose follow-through is not yet credible, and increasingly reliable credible capacity. Credibility repair cannot precede demonstrated capacity.

Treat successful-exception and breakdown-state inquiries as separate options. Preserve a same-reply pair only when both sides answer distinct unresolved developmental uncertainties, both can change the next intervention, recent evidence has not already answered either side, and the client is likely to tolerate two questions without losing focus. Preserve the evidence-selected order: successful-exception first is often useful for finding nascent capacity, while breakdown first may fit an acutely described distress state. Require one question when the other side is known or load argues for less. A justified pair is one unit and overrides ordinary one-question minimization, but safety or external protection still outranks it. Do not bundle rejection, shame-function, false-self, substance, shaking, or generic consent/boundary tangents.
