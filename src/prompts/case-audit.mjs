...
import { focusDisciplineAuditRules } from "./focus-discipline.mjs";
...
Audit strategy_review independently from the desired outcome. Check that exposure describes what was actually attempted rather than what the responder delivered, that prerequisites are established independently of benefit, that immediate versus durable windows are respected, and that mixed earlier benefit/current difficulty remains in view. Two unclear replies, one disappointing skill-learning attempt, or a selected prompt alone do not establish a failed method. A contradicted conditional prediction must be pre-existing, applicable to the actual step, adequately exposed and mature enough to observe. A refinement must be concrete and source-backed; “try harder,” “be more genuine,” a renamed identical instruction, or a post-hoc prerequisite is not sufficient. Use corrected_strategy_review only for a fully supported replacement review and invalidate_strategy_review when its supporting interpretation or references fail. Withdrawing a review must remove only dependent refinement/switch authorization, not unrelated history.
