  ...
  function check(value, rule, name) {
    ...
    if (rule.type === "object") {
      ...
      for (const key of rule.required) {
        // Historical saved v1 tasks predate this additive nullable field. Newly
        // generated structured output supplies it explicitly, including null.
        if (name === "turn_task" && key === "relational_readiness" && !Object.hasOwn(value, key)) continue;
        check(value[key], rule.properties[key], `${name}.${key}`);
      }
    } else if (rule.type === "array") {
      if (!Array.isArray(value) || value.length > (rule.maxItems ?? 12)) throw new ValidationError(`${name} must be a bounded array.`);
...
// A newly evidenced readiness assessment for the new issue may survive this reset;
// inherited permission/prohibition may not.
export function reconcileIssueScope(snapshot, priorSnapshot) {
  ...
  const freshlyAssessed = readiness && snapshot.turn_task.issue === snapshot.current_issue
    ...
    && ([...readiness.stability_observation_ids, ...readiness.harm_observation_ids,
      ...readiness.risk_signals.flatMap(signal => signal.observation_ids)].some(id => freshIds.has(id))
      || (readiness.current_stability === "unknown" && readiness.foreseeable_harm === "unknown"
        && readiness.trajectory === "unknown" && readiness.support_purpose === "unknown"
        && !readiness.risk_signals.length && snapshot.turn_task.observation_ids.some(id => freshIds.has(id))));
  return {
    ...snapshot,
    ...(snapshot.turn_task ? { turn_task: { ...snapshot.turn_task, relational_readiness: freshlyAssessed ? readiness : null } } : {}),
