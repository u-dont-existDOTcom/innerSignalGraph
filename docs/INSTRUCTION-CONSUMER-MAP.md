...
| Validated protective-compatibility assessment and durable restriction state | Case audit, pre-selection planner guard, final response contract, private candidate lifecycle, hypnosis compiler, and advisory plugin skill | Source-bound child-contact permission decision plus present-focused alternatives | Observation-ID validation, actor scope, HOLD/BLOCKED persistence, exact state binding, whole-response rejection/fallback, hypnosis pre-draft gate, repair/discriminator checks, and synthetic regressions |
...
- Protective compatibility and threat urgency remain separate decisions. A child-contact restriction is not proof of imminent danger, and a prompt or model audit is not a clinical-clearance service.
