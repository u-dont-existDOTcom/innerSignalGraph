...
| `inner-signal-private-continuity` activation instructions | Ordinary authorized ChatGPT advisory continuation | Current read-only case tools on each case-related turn | Truthful tool/auth failure disclosure; off-path controlled coverage remains unknown; the skill cannot mutate, approve, send, or establish an independent audit |
