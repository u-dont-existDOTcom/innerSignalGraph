...
The sign-in challenge is kept for denials that signing in again could cure: no token, an invalid one, an account with no case grants, or a token missing a scope the tool needs. A signed-in account that asks for a case or handoff it can't open, or one that doesn't exist, gets a tool error with code `PRIVATE_CASE_NOT_AUTHORIZED` instead. Otherwise a wrong ID from the model would look like a broken sign-in, and Claude marks the connector as needing re-authentication. The error is the same whether or not the case exists.
