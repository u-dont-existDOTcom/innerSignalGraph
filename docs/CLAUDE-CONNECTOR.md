...
The sign-in challenge is kept whenever signing in again could cure a denial: no token, an invalid one, a token missing a scope the tool needs, or a deployment whose case ACL grants more than one account (another account might open the case). When the ACL grants a single account and the request carries that account's valid token, a case or handoff it can't open, or one that doesn't exist, gets a tool error with code `PRIVATE_CASE_NOT_AUTHORIZED` instead. Otherwise a wrong ID from the model would look like a broken sign-in, and Claude would mark the connector as needing re-authentication. The error is the same whether or not the case exists. Deployments with several accounts keep the challenge until the owner decides how account selection should work.
