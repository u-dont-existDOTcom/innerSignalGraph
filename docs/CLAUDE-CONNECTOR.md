...
2. **Resource URL.** The connector URL must be exactly the `resource` value that `/.well-known/oauth-protected-resource` returns (`INNER_SIGNAL_MCP_RESOURCE`). The hosted CLI serves MCP at `/mcp` on that host, so the resource should be that `/mcp` URL; Claude rejects a mismatch.
