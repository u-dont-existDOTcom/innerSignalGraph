...
const canonicalMapPath = path.join(root, "docs", "INNER-CHILD-THERAPY-MAP.md");
const canonicalBaseCommit = "6e3e018a7e11a0c20ce9414af65ebcea93f722bd";
const preservedSkillSha256 = "c6a6ae66930734c2276d8b3d5f930dd4ca22f5b1de17b347b08721d2fb2d379f";
const priorMapSha256 = "29b2d9b02d7017e0c6f9143a2159ebc8bb478a260680d494b4fe0b7b7656700b";
...
function verifyHashes(skillFiles) {
  ...
  if (hashes.baseCommit !== canonicalBaseCommit) fail("package base commit is not the authorized canonical main");
  if (hashes.skillsTreePrePostIdentical !== false) fail("the intentional map refresh is not recorded");
  ...
  for (const item of recordedSkills) {
    ...
    if (item.postSha256 !== actual || item.identical !== (item.preSha256 === item.postSha256)) fail(`skill transition binding is stale: ${item.path}`);
  }
  const skillEntry = recordedSkills.find((item) => item.path.endsWith("/SKILL.md"));
  if (skillEntry?.preSha256 !== preservedSkillSha256 || skillEntry.postSha256 !== preservedSkillSha256 || skillEntry.identical !== true) {
    fail("SKILL.md is not byte-identical to the reviewed PR #20 source");
  }
  const mapEntry = recordedSkills.find((item) => item.path.endsWith("/references/INNER-CHILD-THERAPY-MAP.md"));
  const canonicalMapSha256 = sha256(canonicalMapPath);
  if (mapEntry?.preSha256 !== priorMapSha256 || mapEntry.postSha256 !== canonicalMapSha256 || mapEntry.identical !== false) {
    fail("map transition does not bind the prior package to the current canonical map");
  }
  if (mapEntry.source?.path !== "docs/INNER-CHILD-THERAPY-MAP.md" || mapEntry.source.commit !== canonicalBaseCommit || mapEntry.source.sha256 !== canonicalMapSha256) {
    fail("map source provenance is stale");
  }
  if (!fs.readFileSync(path.join(root, mapEntry.path)).equals(fs.readFileSync(canonicalMapPath))) {
    fail("packaged map is not byte-identical to the canonical generated map");
  }
  ...
  return { manifestSha256: hashes.manifest.sha256, skillFiles: recordedSkills.length, submissionFiles: recordedSubmission.length, mapSha256: canonicalMapSha256 };
}
...
try {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    skillsOnly: true,
    skillEntryPointPreserved: true,
    mapDerivedFromCurrentMain: true,
