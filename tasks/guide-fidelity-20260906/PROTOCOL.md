...
Grader contract v2 records explicit criterion applicability along with pass/revise/block/uncertain, source IDs, exact answer evidence and reasons. An applicable judgment requires actual source evidence. An inapplicable judgment must be an explained pass and may have no citation; any citation supplied must still be real. This makes the existing applicability rule consistent across prompt, schema and validator. The first live calibration exposed that inconsistency; its unmodified, unadmitted output is retained at `tasks/guide-source-sync-20260907/evidence/smoke-attempt-1/` and is not retrospectively scored. Source/quote checks establish provenance, NOT semantic correctness. Omitted behavior can have an empty quote with a specific explanation. Disagreement and source uncertainty require review; a blocker is not averaged away. No model self-grades its answer. Since visible content can still reveal style, identity blinding is procedural, not a guarantee that a model cannot guess.
