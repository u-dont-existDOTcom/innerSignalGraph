...
export const gradeSchema = {
  ...
  properties: {
    criteria:{type:'array',items:{type:'object',additionalProperties:false,properties:{id:{type:'string',enum:CRITERIA},verdict:{type:'string',enum:['pass','revise','block','uncertain']},reason:string,source_ids:{type:'array',items:string},answer_quote:string},required:['id','verdict','reason','source_ids','answer_quote']}},
...
export function validateGrade(grade, {answer, sources}) {
  ...
  for (const c of grade.criteria) {
    ...
    ids.add(c.id);
    if (!Array.isArray(c.source_ids) || !c.source_ids.length || c.source_ids.some(id=>!allowed.has(id))) throw new Error('Grade cites unavailable source');
