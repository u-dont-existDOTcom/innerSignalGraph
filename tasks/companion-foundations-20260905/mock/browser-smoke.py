...
def main():
    ...
    with sync_playwright() as p:
        ...
        check(page.locator('#workspace #history-controls #history').count() == 1, 'History opt-in is beside the examples')
        check(page.locator('aside #history').count() == 0, 'History control is not under invitation preferences')
        check(page.locator('#history-controls #scenario').count() == 1 and page.locator('#history-controls #reflection-pref').count() == 1, 'Related example controls are grouped')
        page.locator('label.check').click()
        check(page.locator('#history').is_checked(), 'History checkbox has a working label')
        ...
        page.locator('details.handoff-line > summary').click()
        page.click('#queue-reflection')
        check(page.locator('#reading').is_hidden(), 'Queue clears the prior reading')
        page.click('#deliver-reflection')
        check(page.locator('#reading').is_visible(), 'Current queued reply can be delivered')
        before = page.locator('#reading-text').inner_text()
        page.click('#deliver-reflection')
        check(page.locator('#reading-text').inner_text() == before, 'Duplicate delivery cannot change the current reading')
        check('blocked' in page.locator('#handoff-status').inner_text(), 'Duplicate delivery is refused')
        page.click('#queue-reflection')
        page.click('#correct-m2')
        check('on leave' in page.locator('#reports').inner_text(), 'Correction replaces the sample report')
        check(page.locator('#reading-text').inner_text() == '', 'Correction removes obsolete interpretation from DOM')
        check(page.evaluate('document.activeElement.id') == 'notice', 'Correction restores keyboard focus')
        page.click('#deliver-reflection')
        check(page.locator('#reading').is_hidden() and 'blocked' in page.locator('#handoff-status').inner_text(), 'A delayed reply cannot survive source correction')
        page.click('#review')
        check(page.locator('#reading').is_hidden(), 'Corrected source needs reassessment, not old scripted prose')
        page.select_option('#scenario', 'boundaries')
        page.click('#queue-reflection')
        page.uncheck('#history')
        page.check('#history')
        page.click('#deliver-reflection')
        check(page.locator('#reading').is_hidden(), 'Re-enabling history does not resurrect an old reply')
        page.click('#queue-reflection')
        page.select_option('#scenario', 'natural')
        page.select_option('#scenario', 'boundaries')
        page.click('#deliver-reflection')
        check(page.locator('#reading').is_hidden(), 'Returning to the same example does not resurrect an old reply')
        page.click('#queue-reflection')
        page.select_option('#reflection-pref', 'off')
        page.select_option('#reflection-pref', 'on_request')
        page.click('#deliver-reflection')
        check(page.locator('#reading').is_hidden(), 'Restoring reflection preference does not restore an old ticket')
        page.click('#queue-reflection')
        page.click('#cancel-reflection')
        page.click('#deliver-reflection')
        check(page.locator('#reading').is_hidden(), 'Cancellation blocks delivery')
        page.click('#queue-reflection')
        page.click('#review')
        before = page.locator('#reading-text').inner_text()
        page.click('#deliver-reflection')
        check(page.locator('#reading-text').inner_text() == before and len(before) > 0, 'A stale reply cannot erase the newer visible reading')
        page.click('#queue-reflection')
        page.click('#exit')
        check(page.locator('#handoff-status').inner_text() == '', 'Exit clears the request status')
        page.click('#reopen')
        page.check('#history')
        page.click('#deliver-reflection')
        check(page.locator('#reading').is_hidden(), 'Exit and restart cannot accept old replies')
        page.click('#queue-reflection')
        page.evaluate("window.dispatchEvent(new Event('pagehide'))")
        check(page.locator('#active').is_hidden() and page.locator('#reading-text').inner_text() == '', 'Page-hide clears the demonstration session')
        page.click('#reopen')
        page.set_viewport_size({'width': 320, 'height': 720})
        check(page.evaluate('document.documentElement.scrollWidth <= window.innerWidth'), 'Narrow mobile has no horizontal overflow')
        page.set_viewport_size({'width': 390, 'height': 844})
        page.check('#history')
        page.select_option('#scenario', 'mixed')
        page.click('#review')
        page.screenshot(path=str(args.out / 'mobile-revised.png'), full_page=True)
        page.set_viewport_size({'width': 1400, 'height': 1100})
        page.screenshot(path=str(args.out / 'desktop-revised.png'), full_page=True)
        ...
        report = {'kind': 'synthetic-ui-smoke', 'passed': len(checks), 'checks': checks,
                  ...
                  'viewports': ['1400x1100', '390x844', '320x720']}
