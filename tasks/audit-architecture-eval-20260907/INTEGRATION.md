...
Status: deterministic harness and owner-frozen ChatGPT-UI smoke plan implemented and locally tested on draft PR #46; the refreshed live calibration stopped on invalid JSON before any architecture smoke; no winning architecture selected.
...
The frozen live run is stopped and cannot resume. One owner decision now precedes a new version: whether to replace free-form grader/validator `reason` strings with a smaller machine-only contract—recommended: verdict/rating plus source span offsets or stable omission target IDs, with any prose explanation stored separately and non-gating—or instead authorize a deterministic JSON-repair/retry policy. The first option is less ambiguous, shorter for batch grading, and avoids silently editing semantic evidence; the second preserves prose in the gating artifact but needs explicit repair rules and consumes retry budget. Either choice versions prompts, schemas, controls, hashes, and calibration. A new run must repeat all six controls; the failed output cannot be discarded or counted as a pass.

Only after a new calibration passes do the previously settled post-run owner decisions return: whether evidence warrants a full corpus, whether `Latest` materially changes eventual-runtime assumptions, and whether any architecture should eventually be adopted. The harness cannot decide or implement those choices.

## Next safe action

Preserve the stopped private run and obtain the single output-contract decision above. If a new version is authorized, implement and deterministically test it, freeze a new containing commit and hashes, then repeat all six calibration controls before any architecture or `Latest` call. Do not retry the failed control under the current run, run a full corpus, select or implement an architecture, infer the backend behind `Latest`, merge, deploy, install, or advance `stable`.
