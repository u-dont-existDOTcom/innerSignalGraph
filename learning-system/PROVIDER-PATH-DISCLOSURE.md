# Provider-path disclosure — offline draft

**Status:** exact future disclosure contract; unpublished and not connected to signup.

## Free user-owned ChatGPT path
...
## Paid InnerSignal-controlled API path
...
## Identifiability warning — both paths
...
Provider-path selection and InnerSignal community-learning contribution are separate choices.
The user-owned ChatGPT path is governed by the user's OpenAI plan and settings; InnerSignal
does not control them. The paid API path is not implemented by this draft.
