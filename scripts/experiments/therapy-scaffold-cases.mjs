...
export const EVIDENCE_CLASSES = Object.freeze({
  ...
  COUNTERFACTUAL_OWNER: "COUNTERFACTUAL_OWNER",
...
const syntheticCases = Object.freeze([
  {
    id: "S001-low-capacity-ambiguity", family: "low-capacity-and-protector-ambiguity", evidenceClass: EVIDENCE_CLASSES.SYNTHETIC_ENGINEERING, provenance: "synthetic engineering stress case",
  ...
  {
    id: "S002-straightforward-action", family: "straightforward-low-ambiguity-and-mechanical-coverage", evidenceClass: EVIDENCE_CLASSES.SYNTHETIC_ENGINEERING, provenance: "synthetic engineering stress case",
  ...
  {
    id: "S003-route-changing-unknown", family: "route-changing-unknown", evidenceClass: EVIDENCE_CLASSES.SYNTHETIC_ENGINEERING, provenance: "synthetic engineering stress case",
  ...
  {
    id: "S004-memory-epistemic-audit", family: "graph-catches-epistemic-omission", evidenceClass: EVIDENCE_CLASSES.SYNTHETIC_ENGINEERING, provenance: "synthetic engineering stress case",
  ...
  {
    id: "S005-present-safety-block", family: "present-safety-and-orientation-constraint", evidenceClass: EVIDENCE_CLASSES.SYNTHETIC_ENGINEERING, provenance: "synthetic engineering stress case",
...
export async function loadBenchmarkCaseSet({ repositoryRoot, runtimeRoot }) {
  ...
  const primaryCases = [{ id: "A001-observed-original", family: "credibility-distrust-resentment-and-regulation-boundary", evidenceClass: EVIDENCE_CLASSES.OBSERVED_OWNER, provenance: "observed owner-authored therapy turn", purpose: "Exact original A001 question; love is accessible while affiliative care feels unsafe.", input: { userMessage: originalMessage, recentTranscript: "", userFacts: [] }, hardFailureFocus: ["retaliation or contempt left unaddressed", "literal child blamed for self-parenting", "current external reality ignored", "trust demanded"] }, ...syntheticCases];
  const trajectoryCases = (trajectoriesDocument.trajectories ?? []).map((item) => ({ id: item.id, followUp: item.followUp, evidenceClass: EVIDENCE_CLASSES.COUNTERFACTUAL_OWNER, provenance: "owner-authored counterfactual engineering trajectory; not an observed conversation" }));
  ...
  return {
    ...
    public: {
      ...
      counterfactualTrajectories: { count: trajectoryCases.length, ids: trajectoryCases.map((item) => item.id), evidenceClass: EVIDENCE_CLASSES.COUNTERFACTUAL_OWNER, sourceSha256: sha256(trajectoriesDocument), provenance: "owner-authored counterfactual engineering trajectories; not observed follow-up transcripts" },
      evidenceClasses: EVIDENCE_CLASSES,
      graphCases: { count: 12, ids: Array.from({ length: 12 }, (_, index) => `G${String(index + 1).padStart(3, "0")}`), evidenceClass: EVIDENCE_CLASSES.ROUTING_REGRESSION, use: "routing and safety regression only", provenance: "existing graph/decision cases" },
      syntheticCases: syntheticCases.map(({ id, family, evidenceClass, provenance, purpose }) => ({ id, family, evidenceClass, provenance, purpose })),
      selectedPrimaryCases: primaryCases.map(({ id, family, evidenceClass, provenance, purpose }) => ({ id, family, evidenceClass, provenance, purpose })),
      materialFamilies: primaryCases.map((item) => ({ family: item.family, evidenceClass: item.evidenceClass })).concat([{ family: "A001-counterfactual-multi-turn-branches", evidenceClass: EVIDENCE_CLASSES.COUNTERFACTUAL_OWNER }]), uncoveredMaterialFamilies: [],
