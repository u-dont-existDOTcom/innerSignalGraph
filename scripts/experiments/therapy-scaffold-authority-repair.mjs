...
export function classifyProviderRateLimit(error) {
  ...
  return {
    provider: failed?.response?.provider ?? failed?.request?.metadata?.provider ?? (failed?.error?.details?.model ? "anthropic" : null),
    requestedModel: failed?.response?.model ?? failed?.error?.details?.model ?? null,
